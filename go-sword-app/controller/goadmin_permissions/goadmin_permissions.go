...
import (
	"encoding/json"
	"go-sword/go-sword-app/model/goadmin_permissions"
	"go-sword/response"
	"io/ioutil"
	"log"
	"net/http"
...
func List(db *gorm.DB) func(writer http.ResponseWriter, request *http.Request) {
	return func(writer http.ResponseWriter, request *http.Request) {
		sqls := []goadmin_permissions.GoadminPermissions{}
		db.Find(&sqls)
...
func Delete(db *gorm.DB) func(writer http.ResponseWriter, request *http.Request) {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		sql := goadmin_permissions.GoadminPermissions{
			ID: data["id"],
		}
...
func Create(db *gorm.DB) func(writer http.ResponseWriter, request *http.Request) {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		data := goadmin_permissions.GoadminPermissions{}
		err = json.Unmarshal(body, &data)
		if err != nil {
			log.Printf("%v", err)
			return
